...
public class TrackManager : MonoBehaviour
{
    ...
    public static Dictionary<string, Answer> pgeAnswers = new Dictionary<string, Answer>();
    ...
    public void StartMove(bool isRestart = true)
    {
        ...
        if (pgeAnswers == null || !PlayerData.instance.tutorialDone) return;

        foreach (var entry in pgeAnswers)
        {
            entry.Value.Use();
        }
    }
    ...
    {
        while (m_Segments.Count > 1)
        {
            int lastIndex = m_Segments.Count - 1;
            Destroy(m_Segments[lastIndex].gameObject);
